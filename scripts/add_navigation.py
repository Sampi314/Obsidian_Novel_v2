...
def generate_navigation(repo_root):
    ...
    for pov in pov_dirs:
        ...
        for i, current_chapter in enumerate(chapter_list):
            ...
            # Previous Button
            if prev_chapter:
                nav_html.append(f'<td style="border: none; padding: 5px;"><a href="{prev_chapter["filename"].replace(".md", "")}">⬅️ Chương Trước</a></td>')
            else:
            ...
            # Next Button
            if next_chapter:
                nav_html.append(f'<td style="border: none; padding: 5px;"><a href="{next_chapter["filename"].replace(".md", "")}">Chương Sau ➡️</a></td>')
            else:
            ...
            for chap in chapter_list:
                ...
                nav_html.append(f'<li style="padding: 5px; {is_active}"><a href="{chap["filename"].replace(".md", "")}">{chap["title"]}</a></li>')
            ...
            try:
                ...
                # Remove ALL existing navigation blocks
                content_without_nav = nav_pattern.sub("", content)
                content_without_nav = content_without_nav.strip()
                ...
                if match:
                    ...
                    end_pos = match.end()
                    new_content = content_without_nav[:end_pos] + nav_block_top + content_without_nav[end_pos:] + nav_block_bottom
                else:
                    # Prepend to top and append to bottom
                    new_content = nav_block_top + content_without_nav + nav_block_bottom
